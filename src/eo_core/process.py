...
from .utils import generate_low_res_preview, get_device
    ...
    try:
        while True:
            ...
            for r_p, c_p in coords:
                ...
                avg_probs[:, r_p : r_p + patch_size, c_p : c_p + patch_size] += (
                    patch_prob
                )
                weight_sum[:, r_p : r_p + patch_size, c_p : c_p + patch_size] += (
                    patch_weight
                )
                idx += 1
        ...
        try:
            generate_single_node_viewer(
                context["tile_name"], str(context["output_path"].parent)
            )
        except Exception as e:
...
def main_hydra(cfg: DictConfig):
    ...
    model_num_frames = OmegaConf.select(
        cfg, "model.adapter.params.backbone_params.num_frames", default=1
    )
    if model_num_frames is None:
        model_num_frames = 1  # Handle explicit None
    ...
    if tile_path.name.endswith(".SAFE"):
        ...
        if len(s2_sub_tiles) > 1 and model_num_frames > 1:
            log.info(
                f"Detected Multi-Temporal S2 Series ({len(s2_sub_tiles)} frames) for Temporal Model (req: {model_num_frames})."
            )
            ...
            if len(s2_sub_tiles) > 1:
                log.warning(
                    f"Found {len(s2_sub_tiles)} S2 products but model is Single-Frame. Using first product as target."
                )
                ref_tile_path = s2_sub_tiles[0]
            else:
                log.info(
                    "No multiple S2 products found. Treating directory as tile root."
                )
                ref_tile_path = tile_path
    ...
    temp_s2_pattern_for_ref = s2_file_pattern.format(
        resolution="10", band_name="{band_name}"
    )
    ...
    if "adapter" in cfg.model:
        ...
        if "patch_size" not in adapter_cfg["params"]:
            adapter_cfg["params"]["patch_size"] = cfg.pipeline.tiling.get(
                "patch_size", 120
            )

        if "stride" not in adapter_cfg["params"]:
            adapter_cfg["params"]["stride"] = cfg.pipeline.tiling.get(
                "patch_stride", 60
            )
        ...
        if means is None or stds is None:
            raise ValueError(
                "Model config must contain 'means' and 'stds' for normalization if no explicit adapter is defined."
            )
    ...
    try:
        ...
        raise ImportError(
            f"Could not load adapter '{class_name}' from '{module_path}': {e}"
        )
    ...
    if "reporters" in cfg.pipeline and cfg.pipeline.reporters:
        ...
        # Pass the DictConfig/ListConfig directly
    ...
    # Calculate ZoR using the model-specific parameters and the full pipeline config
    zor_config = cfg.pipeline.tiling.zone_of_responsibility_size
    zor = resolve_zor(
        zor_config,
        pipeline_cfg=cfg.pipeline,
        patch_size=patch_size,
        max_ram_gb=cfg.pipeline.tiling.get("max_memory_gb"),
        num_bands=num_bands,
    ...
    log.info(
        f"Inference Configuration: ZoR={zor}, Halo={halo}, ChunkSize={zor + 2 * halo}"
    )
    log.info(
        f"Model Params: Patch={patch_size}, Stride={stride}, Bands={num_bands}, Classes={num_classes}"
    )
    log.info(f"Memory Logic: Segmentation={adapter.is_segmentation}")
    ...
    writer_p = ctx.Process(
        ...
        args=(
            ...
            engine.adapter,
            reporter_configs,
            context_config.get("benchmark_report_path"),
        ),
        daemon=True,
    )
    writer_p.start()
    ...
    use_prefetcher = cfg.model.get(
        "use_prefetcher", cfg.pipeline.distributed.get("use_prefetcher", True)
    )
    pq_size = (
        cfg.pipeline.distributed.get("prefetch_queue_size", 2) if use_prefetcher else 0
    )
    ...
    bench_config = {
        ...
        "zor": zor,
        "halo": halo,
        "gpu_batch_size": adapter.gpu_batch_size
        if hasattr(adapter, "gpu_batch_size")
        else "unknown",
    ...
    if use_prefetcher:
        ...
        def get_next_batch():
            ...
            benchmarker.record_event(
                "wait_for_prefetch_duration", time.perf_counter() - t_wait
            )
            return item
    else:
    ...
    try:
        while True:
            ...
            if not writer_p.is_alive():
                log.error(
                    f"Writer process died unexpectedly with exit code {writer_p.exitcode}. Stopping."
                )
                break
            ...
            benchmarker.record_event(
                "wait_for_writer_queue_duration", time.perf_counter() - t_wait_write
            )

            # Explicitly free memory for the current batch
            del model_input
            del logits_t
            gc.collect()
        ...
        if writer_p.exitcode is not None and writer_p.exitcode != 0:
            log.error(
                f"Writer process exited with error code {writer_p.exitcode}. Output files may be incomplete."
            )
