...
\begin{table}[H]
    ...
    \begin{tabular}{|l|c|c|c|c|c|c|}
        \hline
        \textbf{Model} & \textbf{Duration (s)} & \textbf{Throughput (MPix/s)} & \textbf{GPU Util (Avg)} & \textbf{Peak Temp (°C)} & \textbf{Peak VRAM (GB)} & \textbf{Peak RAM (GB)} \\
        \hline
        \textbf{Prithvi-100M} & 348.94s & 0.34 & 75.4\% & 89°C & 3.99 GB & 14.61 GB \\
        \hline
        \textbf{ResNet-50 (All)} & 159.69s & 0.75 & 40.9\% & 89°C & 2.97 GB & 13.70 GB \\
        \hline
        \textbf{ResNet-50 (S2)} & 224.63s & 0.53 & 35.2\% & 88°C & 2.97 GB & 14.54 GB \\
        \hline
        \textbf{ConvNeXt-S2} & 636.11s & 0.19 & 84.0\% & 90°C & 3.47 GB & 14.83 GB \\
        \hline
    \end{tabular}
    }
\end{table}
...
\begin{itemize}
    \item \textbf{Compute Bound (ConvNeXt & Prithvi):} ConvNeXt was the slowest model, taking over 10 minutes to process the tile. It drove the GPU to \textbf{90°C} and maintained 84\% utilization. Prithvi also showed high utilization (75\%), saturating the 4GB VRAM limit of the RTX 3050 (3.99 GB peak usage). This confirms that these heavy models are limited by the GPU's compute capability on this hardware.
